...
# =========================
st.set_page_config(page_title="Predicción de Riesgo de enfermedades - App", layout="wide")
st.title("Predicción de Riesgo de enfermedades - App")
sns.set_theme()
...
# --------- Asegurar split y preprocesador ---------
if 'X_train' not in locals() or 'y_train' not in locals():
    # Split 80/20 estratificado sin muestreo
    drop_cols = ['survey_code', 'bmi_estimated', 'bmi_scaled', 'bmi_corrected']
    X_all = data.drop(columns=[c for c in drop_cols + ['target'] if c in data.columns], errors='ignore')
    y_all = data['target']
    X_train, X_test, y_train, y_test = train_test_split(X_all, y_all, train_size=0.80, stratify=y_all, random_state=42)

# Usa el preprocessor global si existe; si no, crea uno básico
if 'preprocessor' not in locals():
    ...
    preprocessor = ColumnTransformer(
        transformers=[("num", pre_num, num_cols_base), ("cat", pre_cat, cat_cols_base)],
        remainder="drop"
    )
...
results_rows = []
searches = {}  # guardará los RandomizedSearchCV por modelo
cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)

with st.spinner("Entrenando y evaluando modelos..."):
    for name, clf in models.items():
        pipe = ImbPipeline(steps=[
            ('preprocessor', preprocessor),   # asume que ya lo definiste antes
        ...
        search = RandomizedSearchCV(
            ...
            n_iter=10,
            cv=cv,
            scoring='f1_macro',              # favorece F1-macro en CV
            n_jobs=-1,
            verbose=0,
            random_state=42
        )
        ...
        # Reportes (solo métricas agregadas)
        rpt_test  = classification_report(y_test,  y_pred_test,  output_dict=True, zero_division=0)
        rpt_train = classification_report(y_train, y_pred_train, output_dict=True, zero_division=0)

        # Fila de resultados
        results_rows.append({
            "Modelo": name,
            "CV best (F1-macro)": round(search.best_score_, 4),
            "TEST • Accuracy":    round(rpt_test.get("accuracy", 0.0), 4),
            "TEST • F1-macro":    round(rpt_test.get("macro avg", {}).get("f1-score", 0.0), 4),
            "TEST • Precision-m": round(rpt_test.get("macro avg", {}).get("precision", 0.0), 4),
            "TEST • Recall-m":    round(rpt_test.get("macro avg", {}).get("recall", 0.0), 4),
            "TRAIN • Accuracy":   round(rpt_train.get("accuracy", 0.0), 4),
            "TRAIN • F1-macro":   round(rpt_train.get("macro avg", {}).get("f1-score", 0.0), 4),
        })

# --------- Tabla de resultados ---------
leaderboard_df = pd.DataFrame(results_rows).sort_values("TEST • F1-macro", ascending=False).reset_index(drop=True)
st.subheader("Resultados por modelo")
st.dataframe(leaderboard_df, use_container_width=True)

# --------- Mejor modelo por CV (para listar hiperparámetros) ---------
best_cv_row = max(results_rows, key=lambda r: r["CV best (F1-macro)"])
best_model_name = best_cv_row["Modelo"]
best_search = searches[best_model_name]
best_params_full = best_search.best_params_

# Hiperparámetros solo del clasificador (remover prefijo 'classifier__')
classifier_params = {k.split("__", 1)[1]: v for k, v in best_params_full.items() if k.startswith("classifier__")}
# Si buscaste algo de PCA, también (opcional):
pca_params = {k.split("__", 1)[1]: v for k, v in best_params_full.items() if k.startswith("pca__")}

st.subheader("Mejor modelo (según CV F1-macro)")
st.write(f"**Modelo:** {best_model_name}  |  **CV best (F1-macro):** {best_cv_row['CV best (F1-macro)']}")

# --------- Mostrar mejores hiperparámetros en tablas ---------
if classifier_params:
    st.markdown("**Hiperparámetros óptimos del clasificador**")
    st.dataframe(
        pd.DataFrame(sorted(classifier_params.items()), columns=["Hiperparámetro", "Valor"]),
        use_container_width=True
    )
else:
    st.info("No se encontraron hiperparámetros del clasificador (o no se ajustaron en la búsqueda).")

if pca_params:
    st.markdown("**Hiperparámetros de PCA (si aplica)**")
    st.dataframe(
        pd.DataFrame(sorted(pca_params.items()), columns=["Hiperparámetro", "Valor"]),
        use_container_width=True
    )
